...
**Decision:** the Cache tracks one dirty bit per resident layer plane per spatial slot, not per cell. A changed `Cache::set<LayerT>(position, value)` marks the whole resident layer plane dirty; a same-value set is a no-op on a clean plane and a plane that is already dirty stays dirty. The only dirty-clearing path is the write-back hook `mark_clean<LayerT>()` (D-38), which Map's explicit `flush<LayerT>()` calls only after a plane has been fully persisted: no fill, invalidation or placement operation clears dirt, so a dirty plane survives every other operation until it is flushed or the Cache is destroyed. `fill<LayerT>()` refuses to overwrite a dirty target plane and returns `false` without writing. Both invalidation forms are atomic and return a `bool`: if any slot that would be discarded contains a dirty resident plane, the entire operation is refused and the Cache remains unchanged; a dirty slot outside the invalidated area does not block `invalidate(area)`.
...
- the bounded Cache holds explicitly populated resident state and has no eviction policy; write-back exists only as the explicit `flush<LayerT>()` of D-38 (no automatic flush, no replacement under dirty pressure, both still open stop conditions); discarding a mutated value to make room would silently lose working world state, so the safe default remains refusal, not replacement;
...
- once dirty, a plane stays dirty because no operation could legitimately remove the dirt except persisting it (D-38); a sticky bit makes every consumer's decision (fill, invalidate, placement mutation) total without any flush bookkeeping;
...
## D-38 — Explicit dirty-state write-back through the bound runtime source
