...
Map also borrows the runtime layer binding catalogue, a `std::span<const RuntimeLayerBinding>` from `src/world/runtime_layer_source.hpp` scoped to this Map instance: a binding `{ layer, source }` identifies the logical runtime/materialised overlay source of `(Map::id(), layer)` in the runtime storage role (D-35). The constructor asserts the catalogue precondition — every binding names a layer supported by the Map type, and no `LayerId` appears twice in the span; a duplicate is invalid configuration, not a precedence case. Zero bindings means this Map currently has no persistent runtime overlay source for those layers, which is not the same as an unsupported layer. Checked reads consult the catalogue (D-36): when a binding names `LayerT`, `Map::runtime_binding<LayerT>()` returns it and, when the bound source physically exists, the bound runtime source is opened and validated once per missing layer Chunk, with non-space cells authoritative over authored state; a bound source that does not exist yet is a reserved, unmaterialized identity (D-39) and the runtime pass is skipped; when no binding names the layer the runtime storage role is never inspected. The only current write path is the explicit write-back: `flush<LayerT>()` consults the catalogue to find the bound runtime source before persisting a dirty plane, materializing a bound-but-absent source lazily on the first genuine dirty difference (D-38, D-39). The backing records outlive the Map.
...
- when a runtime binding names the layer, the bound source's physical presence is checked through the runtime storage role (`exists()`), and when it exists the bound runtime source is then opened exactly once and validated exactly once against the complete Map geometry (`validate_runtime_layer_source()`); the unresolved in-Map cells are read one at a time through `read_cells()`, with local coordinates computed as `world - map.area().min()` in wide `std::int64_t` before the explicit `std::uint32_t` conversion, and each non-space runtime cell resolves its cell (space leaves the cell unresolved); a reserved-but-unmaterialized source (D-39) and a missing binding both skip this pass (the latter without inspecting the runtime storage);
...
Result exists(SourceId, bool&) const;
Result create(SourceId, Size, std::byte);
```

The contract is logical and all-or-fail. `exists()` reports absence as success with `false`; a present non-regular file or a state the backend cannot determine is a failure. `create()` makes a missing source of the exact requested size filled with one initial value, never replaces an existing source, and removes any partial file on failure.
...
The current `.cpp` implements `read()`, `write()`, `size()`, `exists()`, and `create()`.
...
- `exists()` treating an absent source as success-with-false, a directory at the source path as a failure, and a present regular file as success-with-true;
- `create()` making a missing source of the exact requested size filled with one initial value (in 4 KiB blocks), reporting `already_exists` for a present source, creating parent directories as needed, and removing any partial file on failure;
...
tests/world/test_map_persistence.cpp
...
2. add the replacement policy only after the no-eviction path is proven (explicit per-layer write-back already exists as `Map::flush<LayerT>()`/`flush_all()`, D-38, including lazy materialization of bound-but-absent sources, D-39; replacement under dirty pressure and flush scheduling remain open);
